...
def search_ai_results(
    ...
    # Format results with file information for frontend compatibility
    formatted_results = []
    for result in records:
        file_upload = db.get(FileUpload, result.file_upload_id)
            ...
            try:
                ...
                if isinstance(parsed_metadata, dict):
                    metadata = {
                        ...
                        "recommendations": parsed_metadata.get("recommendations"),
                        "gaps": parsed_metadata.get("gaps"),
                        "esg_alignment": parsed_metadata.get("esg_alignment"),
                        "compliance_indicators": parsed_metadata.get("compliance_indicators"),
                    }
                ...
                logging.getLogger(__name__).warning(f"Failed to parse metadata for result {result.id}: {e}")
                completeness_status = "Unknown"
        else:
            completeness_status = "Unknown"
...
def get_ai_result(
    ...
    try:
        ...
        # Check permissions - auditors can only see their own results
        if current_user.role == "auditor" and result.user_id != current_user.id:
        ...
        logging.getLogger(__name__).error(f"Error getting AI result {result_id}: {str(e)}")
